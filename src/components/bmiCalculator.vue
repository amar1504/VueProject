<template>
   <div class="card " style="width: 25rem; margin-left:25rem;">
        <div class="card-body">
            <h5 class="card-title text-muted" >BMI Calculator</h5>
                ...
                <div class="form-group text-center">
                    <button type="submit" class="btn btn-danger" @click="Calculate();" >Calculate BMI</button>
                </div>
                <div class="mt-5">
                    <VueSpeedometer needleColor="black"  needleTransition="easeElastic" needleTransitionDuration="3000" :value="+bmi" :segments="4" style="height: 210px;"   :customSegmentStops="[0,25,50, 75, 100]" :segmentColors='["firebrick","tomato", "gold", "limegreen"]'  :maxValue="100" />
                    <p v-if="bmi > 0">{{ msg }}</p>
                </div>
        </div>
    </div>
